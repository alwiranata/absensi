...
$role_user = $_SESSION['role']; // pastikan session sudah dimulai di file sebelumnya
?>
...
<div class="fixed border-r p-3 z-0 border-gray-300 bg-white h-screen">
    ...
    <nav class="mt-10">
        ...
        <a href="dashboard.php?page=absensi">
            <div class="flex flex-row mt-6 items-center w-[200px] h-[50px] gap-3 p-3 rounded-[15px] cursor-pointer
        <?php echo ($current_page == 'absensi' || $current_page == 'detail' || $current_page == "edit" || $current_page == "add" ||$current_page == "rekap")
        ...
        <?php if ($role_user === 'admin'): ?>
            <a href="dashboard.php?page=profile">
                <div class="flex flex-row mt-6 items-center w-[200px] h-[50px] gap-3 p-3 rounded-[15px] cursor-pointer
                <?php echo ($current_page == 'profile' || $current_page == 'add_user' || $current_page == "edit_user") ? 'bg-[#E99724] text-white' : 'hover:bg-gray-200'; ?>">
